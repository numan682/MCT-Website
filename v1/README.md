...
- **Hero**: Left side holds the headline, CTA buttons, and stat tiles; right side runs the Swiper slider with lab snapshots. GSAP animates the entry sequence.
- **Highlights**: A mix of feature cards and a spotlight card. Bootstrap grid (`row`/`col-`) manages the alignment; custom glass cards handle the styling.
...
## Styling approach
- Theme tokens live at the top of `assets/css/styles.css` (`:root`) for colors, shadows, and radius. The page background mixes gradients and a subtle noise layer for texture.
...
- Swiper handles the hero slides with autoplay and pagination.
...
## Form handling
- The contact form uses native validation cues with a small custom check in `handleForm()`. Empty required fields get an invalid state, the email is matched against a simple pattern, and a status message is shown inline. No submission is sent; it’s a front-end-only website.
